...
class Hellburner:
    ...
    # ==============================================================================================
    # [HÀM KHÓ / DIFFICULT FUNCTION 1] intercept_planet
    # ----------------------------------------------------------------------------------------------
    # - Mục tiêu: Tính góc bắn và điểm đón (intercept) của hành tinh mục tiêu đang quay quanh mặt trời.
    # - Cơ chế: Do hành tinh mục tiêu di chuyển, thời gian bay (travel) phụ thuộc vào khoảng cách tới vị trí
    #   tương lai của nó. Ta giải phương trình phi tuyến bằng phương pháp lặp (fixed-point iteration).
    #   Để tránh dao động (oscillation) và hội tụ nhanh hơn, ta áp dụng kỹ thuật giảm chấn (dampened update)
    #   bằng cách lấy trung bình cộng của thời gian bay cũ và mới.
    # ----------------------------------------------------------------------------------------------
    # - Goal: Calculate launch angle and interception point for a target planet orbiting the sun.
    # - Mechanism: Since the target moves, travel time depends on the distance to its future position.
    #   We solve this non-linear equation using fixed-point iteration.
    #   To suppress oscillation and ensure convergence, we apply a dampening update technique by averaging
    #   the old and new travel times.
    # ==============================================================================================
    def intercept_planet(
        ...
        if orb is None:
            # Target is static, no iteration needed.
            tx, ty = target.x, target.y
            travel = distance((sx, sy), (tx, ty)) / speed
        else:
            cx = cy = CENTER
            r, ia = orb
            # Seed estimate: straight-line travel time to the planet's current position.
            travel = distance((sx, sy), (target.x, target.y)) / speed
            for _ in range(max_iters):
                # Calculate future angle at current estimated arrival step.
                a = ia + self.angular_velocity * (self.scene_step + travel - 0.5)
                new_tx, new_ty = cx + r * math.cos(a), cy + r * math.sin(a)
                # Compute new travel time based on updated distance.
                ...
                # Diverged: fleet speed is too slow to catch the planet's orbital movement.
                return 0.0, target.x, target.y, math.inf
            # Recompute final position from converged travel time.
            a = ia + self.angular_velocity * (self.scene_step + travel - 0.5)
    ...
    # ==============================================================================================
    # [HÀM KHÓ / DIFFICULT FUNCTION 2] first_planet_hit
    # ----------------------------------------------------------------------------------------------
    # - Mục tiêu: Xác định hành tinh đầu tiên bị hạm đội đâm phải trên đường bay, hoặc kiểm tra xem hạm
    #   đội có đâm vào mặt trời (bị hủy diệt) trước khi tới mục tiêu không.
    # - Cơ chế: Ta duyệt qua tất cả các hành tinh, tính góc đón và kiểm tra xem góc bắn thực tế nằm trong
    #   hình nón va chạm (collision cone) của hành tinh đó hay không. Ta lấy hành tinh có thời gian chạm
    #   nhỏ nhất. Cuối cùng, kiểm tra xem đường bay tới hành tinh đó có cắt qua Mặt Trời hay không.
    # ----------------------------------------------------------------------------------------------
    # - Goal: Determine the first planet hit by the fleet along its launch trajectory, or check if the
    #   path is blocked by the sun (instant destruction) before reaching the target.
    # - Mechanism: Iterates over all planets, computes the intercept angle, and checks if the launch angle
    #   falls within the target planet's collision cone. The planet with the smallest arrival time is chosen.
    #   Finally, checks if the line segment from source to the first hit planet intersects the Sun's radius.
    # ==============================================================================================
    ...
    # ==============================================================================================
    # [HÀM KHÓ / DIFFICULT FUNCTION 3] simulate_planet_timeline
    # ----------------------------------------------------------------------------------------------
    # - Mục tiêu: Mô phỏng diễn biến số lượng tàu và quyền sở hữu của một hành tinh theo thời gian dựa
    #   trên danh sách các hạm đội đang bay tới.
    # - Cơ chế: Gom nhóm các đợt cập bến theo từng lượt (turn). Duyệt qua từng lượt, cộng số tàu sản sinh
    #   (nếu hành tinh được sở hữu), giải quyết giao tranh giữa các thế lực đến đồng thời ở lượt đó,
    #   rồi phân định quyền sở hữu mới.
    # ----------------------------------------------------------------------------------------------
    # - Goal: Simulate the evolution of ship counts and ownership on a planet over time given incoming fleets.
    # - Mechanism: Group arrivals by turn. Step through each turn sequentially, update garrison with planet
    #   production, resolve same-turn combat among arriving fleets (the largest fleet wins after subtracting
    #   runner-up sizes), and update the garrison ownership.
    # ==============================================================================================
    ...
    # ==============================================================================================
    # [HÀM KHÓ / DIFFICULT FUNCTION 4] evaluate_frontline_strategy
    # ----------------------------------------------------------------------------------------------
    # - Mục tiêu: Tính toán xem có thể huy động quân từ các hành tinh lân cận để phòng thủ hoặc tấn công
    #   hành tinh mục tiêu một cách hiệu quả hay không.
    # - Cơ chế: Sắp xếp các hành tinh nguồn lân cận theo cự ly. Với mỗi hành tinh nguồn, ta mô phỏng kịch
    #   bản tệ nhất nếu nó rút bớt tàu (đối thủ có thể phản công nó). Nếu hành tinh nguồn vẫn an toàn hoặc
    #   mục tiêu quan trọng hơn nhiều, ta lên kế hoạch gửi tàu, kiểm tra va chạm vật lý và vật cản mặt trời,
    #   sau đó thử nghiệm gom quân và tối ưu hóa số tàu gửi đi (giảm số tàu gửi nếu thắng thừa thãi).
    # ----------------------------------------------------------------------------------------------
    # - Goal: Calculate if mobilizing ships from nearby owned planets can successfully defend or capture a target.
    # - Mechanism: Sorts neighboring owned candidate sources by distance. For each source, simulates the worst-case
    #   counter-attack to see if withdrawing ships leaves the source vulnerable. If the source remains safe (or the
    #   target production is significantly higher), plans a launch, checks for physical obstacle/sun collisions,
    #   aggregates arrivals, and optimizes the launch size by trimming excess ships if the battle is won comfortably.
    # ==============================================================================================
    ...
    # ==============================================================================================
    # [HÀM KHÓ / DIFFICULT FUNCTION 5] run_early_game
    # ----------------------------------------------------------------------------------------------
    # - Mục tiêu: Tính toán chuỗi tấn công/chiếm đóng các hành tinh trung lập tối ưu trong các lượt đầu tiên.
    # - Cơ chế: Sử dụng thuật toán DFS nhánh và cận (Branch and Bound). Ta dự báo điểm cận trên (upper bound)
    #   tiềm năng ở mỗi trạng thái để cắt tỉa bớt các nhánh tìm kiếm không hiệu quả, tìm ra chuỗi hành tinh
    #   neutral đem lại lợi ích lớn nhất về sản lượng và quân số cuối cùng.
    # ----------------------------------------------------------------------------------------------
    # - Goal: Calculate the optimal expansion sequence to capture neutral planets in the initial rounds.
    # - Mechanism: Implements a branch-and-bound Depth-First Search (DFS). We forecast the potential upper bound
    #   score at each state to prune sub-optimal search paths, discovering the capture sequence that yields
    #   the maximum production and garrison by the look-ahead horizon.
    # ==============================================================================================
    ...
    def main(self, obs: dict[str, Any]) -> list[Any]:
        # Track initial start execution time
